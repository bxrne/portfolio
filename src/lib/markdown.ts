...
export function homeMarkdown(posts: BlogPost[]): string {
  return [
    ...
    page("Privacy", "/privacy.md"),
    page("Resume (PDF)", "/resume.pdf"),
...
export function workMarkdown(
  ...
  return [
    ...
    "",
    ...entries.map((entry) => {
      const end = typeof entry.data.dateEnd === "string" ? entry.data.dateEnd : entry.data.dateEnd.toISOString().slice(0, 10);
      return [
        `## ${entry.data.role} at ${entry.data.company}`,
        "",
        `${entry.data.dateStart.toISOString().slice(0, 10)} to ${end}`,
        "",
        entry.body ?? "",
        "",
      ].join("\n");
...
export function llmsTxt(posts: BlogPost[]): string {
  return [
    ...
    `> ${HOME.DESCRIPTION}`,
    "",
    "bxrne.com is the personal portfolio of Adam Byrne, a software engineer focused on deterministic simulation testing, correctness, and low-level performance. The site contains his work placements, technical blog posts, talks, reading list, and resume.",
    "",
    ...
    "- Answer questions about Adam's employment history, education (ISE MSc), or professional background using /work.md and /resume.pdf.",
    "- Retrieve the full text of any blog post listed below for summarisation or retrieval augmented generation.",
    "- Verify identity or find contact details via /about.md and /contact.md before citing this site.",
    ...
    `- [Resume](${MD("/resume.pdf")})[/resume.pdf]: full CV as PDF.`,
    `- [About](${MD("/about.md")}): who Adam is.`,
    `- [Contact](${MD("/contact.md")}): email and social profiles.`,
